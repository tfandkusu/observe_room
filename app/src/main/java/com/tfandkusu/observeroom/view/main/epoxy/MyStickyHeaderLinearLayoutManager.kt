 ...
 * ```
 */
class MyStickyHeaderLinearLayoutManager @JvmOverloads constructor(
    context: Context,
     ...
     */
    private fun fixHeaderViews() {
        // タグ文字列とViewリストのマップ
        val map: MutableMap<String, MutableList<View>> = HashMap()
        // すべての子Viewに対して
        val childCount = childCount
        for (i in 0 until childCount) {
            val child = getChildAt(i)
